import {  initialStateTweet } from "../../Config/types";
...
const initialState : initialStateTweet = {
    loading : false,
    data : null,
    error : null,
    tweets : [],
...
export const tweetReducer = (state = initialState, action : any) => {
    switch (action.type) {
        ...
        case TWEET_DELETE_SUCCESS:
            return {...state, loading: false, error: null, tweets: state.tweets.filter((tweet : any) => tweet.id !== action.payload)};
